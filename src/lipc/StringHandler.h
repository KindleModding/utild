...
template <typename T> using SimpleStringSetterCB = LIPCcode(StringHandler<T> *_this, LIPC *lipc, std::string value);

template <typename T> using SimpleStringGetterCB = LIPCcode(StringHandler<T> *_this, LIPC *lipc, char **value);

template <typename T> class StringHandler : public IHandler {
  public:
    StringHandler(const std::string &command) : command(command), data(nullptr) {
    }
    static LIPCcode propGetCallback(LIPC *lipc, const char *property, void *value, void *data) {
        auto _this = static_cast<StringHandler<T> *>(data);
        if (_this->getter_cb) {
            return _this->getter_cb(_this, lipc, reinterpret_cast<char **>(value));
        }
    ...
    static LIPCcode propSetCallback(LIPC *lipc, const char *property, void *value, void *data) {
        auto _this = static_cast<StringHandler<T> *>(data);
        if (_this->setter_cb) {
            return _this->setter_cb(_this, lipc, reinterpret_cast<char *>(value));
        }
        return LIPC_ERROR_NO_SUCH_PROPERTY;
    }
    ...
    StringHandler *subscribe(LIPC *lipc) {
        LipcRegisterStringProperty(lipc, this->command.c_str(), this->propGetCallback, this->propSetCallback, this);
        return this;
    };
    void setData(T *data);
    T *getData();
